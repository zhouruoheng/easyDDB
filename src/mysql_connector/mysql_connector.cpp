...
vector<string> db::mysql::string2list(string inputStr, char split)
{
    ...
    vector<string> temp;
    string cur;
    while(getline(Str, cur, split)) temp.push_back(cur);
    return temp;
}
...
string db::mysql::select_sql(string site,string sql)
{
    int PORT;
    char* UNIX_SOCKET;
    if (site=="ddb1") PORT=3316,UNIX_SOCKET="/home/mysql1/mysql.sock";
    else PORT=3306,UNIX_SOCKET="/home/mysql1/mysql.sock";
    MYSQL connection;
    ...
    if (mysql_real_connect(&connection,"127.0.0.1","root","12345678","ddb", PORT, UNIX_SOCKET, CLIENT_FOUND_ROWS)) 
    {
        ...
        while (row=mysql_fetch_row(result)){
            ...
            string temp="(";
            for (int i=0;i<mysql_num_fields(result);i++) temp=temp+"'"+row[i]+"'"+",";
    ...
    else cout<<"Mysql connection fail!"<<endl;
    mysql_close(&connection);
    return {};
}
...
int main()
{
    ...
    cout<<create_insert_delete_sql("dd0", "insert into publisherTemp values " + query_res )<<endl;
